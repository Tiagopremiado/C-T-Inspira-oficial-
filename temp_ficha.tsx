...
import { X, Send, Download, FileText, User, Users, Shield, HeartPulse, Edit2, Save, Printer } from 'lucide-react';
...
export const FichaAluno: React.FC<FichaAlunoProps> = ({
  ...
  const getFieldValue = (keyPre: keyof PreCadastro, keyFull?: keyof CadastroCompleto) => {
    if (isEditing && editData[keyPre] !== undefined) return editData[keyPre];
    if (hasFull && keyFull && cadastroCompleto![keyFull]) return cadastroCompleto![keyFull];
    return preCadastro[keyPre] || 'Não informado';
  };
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 print:overflow-visible print:block">
          ...
          {(activeTab === 'pessoais' || typeof window !== 'undefined' && window.matchMedia('print').matches) && (
            <div className="animate-in fade-in duration-300 print:mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Nome Completo" value={hasFull ? cadastroCompleto!.nome : preCadastro.nomeAluno} />
                <Field label="Data de Nascimento" value={hasFull ? cadastroCompleto!.nascimento : preCadastro.nascimentoAluno} />
                <Field label="WhatsApp" value={hasFull ? cadastroCompleto!.whats : preCadastro.whatsAluno} />
                <Field label="E-mail" value="Não coletado no formulário atual" />
                <Field label="Cidade" value={hasFull ? cadastroCompleto!.cidade : preCadastro.cidadeAluno} />
          ...
          {(activeTab === 'responsaveis' || typeof window !== 'undefined' && window.matchMedia('print').matches) && (
            <div className="animate-in fade-in duration-300 print:mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Nome Responsável" value={hasFull ? cadastroCompleto!.responsavel : preCadastro.nomeResponsavel} />
                <Field label="Parentesco" value={hasFull ? cadastroCompleto!.parentesco : preCadastro.parentesco} />
                <Field label="WhatsApp Responsável" value={hasFull ? cadastroCompleto!.whatsResponsavel : preCadastro.whatsResponsavel} />
...
const Field = ({ label, value, colSpan = false }: { label: string; value: any; colSpan?: boolean }) => {
